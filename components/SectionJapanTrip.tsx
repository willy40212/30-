    ...
    <section id="trip" className="relative py-32 overflow-hidden group">
      ...
      <div className="max-w-4xl mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <span className="text-[#c29967] tracking-[0.5em] font-bold text-xs block mb-6 uppercase drop-shadow-sm">特別企劃</span>
          {/* 調整行距：增加 leading-[1.6] 使主副標題間更有呼吸感 */}
          <h2 className="serif text-4xl md:text-5xl text-white mb-8 leading-[1.6] drop-shadow-2xl">
            日本優雅老後生活體驗：<br />
            <span className="text-stone-200 text-3xl md:text-4xl opacity-90 block mt-6">走進《30》紀錄片的共生場景</span>
          </h2>
        ...
        <div className="max-w-3xl mx-auto text-center">
          <p className="text-lg md:text-xl leading-relaxed text-stone-50 mb-14 serif italic drop-shadow-lg opacity-95">
            您是否也被神戶「はっぴーの家」那種打破圍牆、充滿歡笑與溫度的生活方式所打動？我們邀請您一同開啟這場翻轉想像的生活實驗！
          </p>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
            {/* 輕量級磨砂感卡片：背景不透明度稍微調高 (30%) 並降低模糊感，確保內容在模糊背景上依然顯眼 */}
            <div className="bg-white/30 backdrop-blur-sm p-10 rounded-sm border border-white/30 group/card hover:border-[#c29967]/70 hover:bg-white/40 transition-all duration-500 shadow-[0_12px_40px_0_rgba(0,0,0,0.5)]">
              <span className="text-[#c29967] text-xs mb-3 block font-bold tracking-widest uppercase opacity-100">預計時間</span>
              <span className="text-3xl serif text-white font-bold group-hover/card:text-[#c29967] transition-colors drop-shadow-md">115 年 6 月</span>
            </div>
            <div className="bg-white/30 backdrop-blur-sm p-10 rounded-sm border border-white/30 group/card hover:border-[#c29967]/70 hover:bg-white/40 transition-all duration-500 shadow-[0_12px_40px_0_rgba(0,0,0,0.5)]">
              <span className="text-[#c29967] text-xs mb-3 block font-bold tracking-widest uppercase opacity-100">行程安排</span>
              <span className="text-3xl serif text-white font-bold group-hover/card:text-[#c29967] transition-colors drop-shadow-md">七天六夜深度遊</span>
            </div>
          </div>
